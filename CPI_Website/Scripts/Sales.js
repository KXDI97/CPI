...
let csClient = null;   // #ns-client
let csStatus = null;   // #se-status
...
async function generatePDF(sale) {
  ...
  try { const img = await loadImageAsBase64(COMPANY.logo); doc.addImage(img, "PNG", 14, 10, 30, 15); } catch(_) {}

  doc.setFontSize(10); doc.setTextColor(120);
  doc.text(COMPANY.name, 50, 15); doc.text(`NIT: ${COMPANY.nit}`, 50, 20);
  doc.text(COMPANY.address, 50, 25); doc.text(COMPANY.email, 50, 30);
  ...
  doc.setFontSize(11); doc.setTextColor(40); doc.text("Cliente", 14, 82);
  doc.setFontSize(10); doc.setTextColor(80); doc.text(clientName, 14, 88);
  if (clientDoc) doc.text(clientDoc, 14, 94);
  doc.setDrawColor(200); doc.line(14, 100, 196, 100);
  doc.setFontSize(10); doc.setTextColor(40); doc.setFont(undefined, "bold");
  doc.text("Producto", 14, 110); doc.text("Cant.", 110, 110);
  doc.text("Precio", 135, 110); doc.text("Total", 170, 110);
  doc.setFont(undefined, "normal"); doc.line(14, 112, 196, 112);
  let y = 120;
  (sale.details || []).forEach(d => {
    const prod = allProducts.find(p => p.productId === d.productId);
    doc.setTextColor(60);
    doc.text(prod ? prod.name : d.productId, 14, y);
...
function initHistorySearch() {
  ...
  input.addEventListener("input", function () {
    ...
    let visible = 0;
    rows.forEach(row => {
      const match = row.cells[0]?.textContent.trim().toLowerCase().includes(query);
...
// ===== Helpers select con CustomSelect =====
...
function fillProductsSelect(sel, selectedId = "") {
  ...
  // El MutationObserver del CustomSelect detecta el cambio automáticamente
}
...
function openEditSaleModal() {
  ...
  fillClientsSelect();
  document.getElementById("ns-client").value = sale.clientId;
  csClient?.rebuild();
...
function addLine(prefill = null) {
  ...
  const csLine = new CustomSelect(sel);  // CustomSelect por línea
  ...
  btnDel.addEventListener("click", () => { csLine.destroy(); tr.remove(); renderTotals(); });
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // CustomSelect para #se-status (Sale Status card, siempre visible)
  const seStatusEl = document.getElementById("se-status");
  if (seStatusEl) csStatus = new CustomSelect(seStatusEl);
